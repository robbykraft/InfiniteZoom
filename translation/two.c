...
void drawHUD(){
	...
	// translation data
	glColor3f(1.0, 1.0, 1.0);
	char transString[50], transReport1[50], transReport2[50];
	sprintf(transString, "TRANS: %.5f", transLinear);
	text(transString, thirdW*2.5-50, 18, 0);
	sprintf(transReport1, "+/-dT: %.9f", TRANSLATE_INTERVAL);
	text(transReport1, thirdW*2.5-50, 37, 0);
	sprintf(transReport2, "%.2f*%d^%ld: %.4f", transLinear, INTERVAL, zoomWhole, transExp);
	text(transReport2, thirdW*2.5-50, 56, 0);
...
// process keyboard
	...
	if(keyboard[LEFT_KEY] || keyboard['A'] || keyboard['a']){
		transLinear -= TRANSLATE_INTERVAL;
	}
	if(keyboard[RIGHT_KEY] || keyboard['D'] || keyboard['d']){
		transLinear += TRANSLATE_INTERVAL;
	}
	transExp = transLinear * powf(INTERVAL, zoomWhole);
}
